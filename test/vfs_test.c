...
void vfs_test_seek_file(const char* path, int offset, uint64_t n)
{
	...
	if (f != NULL && buf != NULL && vfs_seek(f, offset, SEEK_SET) >= 0) {
		rd = vfs_read(f, buf, n);
...
/* 写入/读回校验一块数据 */
static int test_write_readback(const char* path, const char* data, uint32_t len)
{
	int fd;
	struct file* f;
	char* buf;
	...
	if (f != NULL && buf != NULL) {
		...
		if (n == (int64_t)len && memcmp(buf, data, len) == 0) {
			ok = 1;
		} else {
...
void test_fat32_operations(void)
{
	...
	const uint32_t msg_len =
	    sizeof("Hello FAT32! loeux riscv vfs test.") - 1;
	int fd;
	struct file* f;
