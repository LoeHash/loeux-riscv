...
#define LS_BUF_ENTRIES                                                         \
	8 /* 8 × sizeof(struct dirent) ≈ 2.2KB，用户栈 4 页够用 */
...
static void mode_string(uint32_t mode, char out[11])
{
	...
	static const uint32_t bits[] = {S_IRUSR,
					S_IWUSR,
					S_IXUSR,
					S_IRGRP,
					S_IWGRP,
...
static void
join_path(char* dst, uint32_t cap, const char* dir, const char* name)
{
	if (strcmp(dir, ".") == 0) {
