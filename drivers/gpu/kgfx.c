 ...
 *   - 写 fb 与标脏一律用resource 绝对坐标：py = screen_y + pan_y。
 *
 * 滚屏
...
/* 定时刷新间隔（拍）。100Hz 下 2 拍 = 50fps，滚屏阅读流畅且省 DMA */
#define KGFX_FLUSH_INTERVAL_TICKS 2
...
void kgfx_clear(struct virtio_gpu_device *gpu, uint32_t color)
{
        ...
        memset(gpu->fb, (int)(uint8_t)color,
...
void kgfx_scroll_up(struct virtio_gpu_device *gpu, uint32_t line_h, uint32_t bg)
{
         ...
         * 落在 backing 高位、不在新窗口内——脏区直接【重置】为单屏，
                    ...
                    uint32_t w, uint32_t h, uint32_t color)
{
        ...
        for (uint32_t row = y0; row < y1; row++) {
                uint32_t *line = (uint32_t *)gpu->fb
                                 + (uint64_t)(row + pan) * gpu->width;
                for (uint32_t col = x0; col < x1; col++)
                        line[col] = color;
        }

        mark_dirty(x0, y0 + pan, x1 - x0, y1 - y0);
}
...
static int kgfx_take_frame(uint32_t *x0, uint32_t *y0,
                          ...
                          uint32_t x, uint32_t y, uint32_t fg, uint32_t bg)
{
        ...
        for (int row = 0; row < ASCII8X16_H; row++) {
                ...
                uint32_t *line = fb + (uint64_t)(sy + pan) * W;
                    ...
                    uint32_t x, uint32_t y, uint32_t fg, uint32_t bg)
{
        ...
        mark_dirty(x, y + kgfx_pan(), ASCII8X16_W, ASCII8X16_H);
}
                      ...
                      uint32_t x, uint32_t y, uint32_t fg, uint32_t bg)
{
        ...
        if (x > start_x)
                mark_dirty(start_x, y + kgfx_pan(),
                           x - start_x, ASCII8X16_H);
}
