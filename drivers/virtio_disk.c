...
// 全局驱动
struct block_driver virtio_block_driver = {.read = virtio_blk_read,
...
uint32_t virtio_disk_rw_sync(struct virtio_blk_disk* selected_desk,
			     struct virtio_blk_req* req,
			     void* buf,
			     uint32_t bytes,
			     uint8_t* status)
{
	...
	vq.desc_start[next_tmp_idx].len =
	    sizeof(struct virtio_blk_req);			// 请求头长度
	...
	if (req->type == VIRTIO_BLK_T_IN) {
		...
		// 并写入到设备
		// 因此设备flag 加上写
		vq.desc_start[next_tmp_idx].flags |= (VRING_DESC_F_NEXT);
	}
	...
	// 通知设备
	b32_write(selected_desk->base_addr + VIRTIO_MMIO_QUEUE_NOTIFY_OFFSET,
		  0); // 0 队列有任务
	...
	if (*status != 0) {
		// panic(PANIC_ERROR,
		// "virtio_blk_disk.c:virtio_disk_rw_sync:panic the status:
		// %d\n", *status);
...
static void detect_disk()
{
	...
	for (int i = 0; i < 8; i++) {
		...
		usable_disks[i].magic =
		    *R_LEVEL(VIRTIO_MMIO_MAGIC_VALUE_OFFSET, i);
		// version
		usable_disks[i].version =
		    *R_LEVEL(VIRTIO_MMIO_VERSION_OFFSET, i);
		// device id
		usable_disks[i].device_id =
		    *R_LEVEL(VIRTIO_MMIO_DEVICE_ID_OFFSET, i);
		// vendor
		usable_disks[i].vendor =
		    *R_LEVEL(VIRTIO_MMIO_VENDOR_ID_OFFSET, i);
		// virtio addr
		...
		if (usable_disks[i].version == 2) {
			...
			*R_LEVEL(VIRTIO_MMIO_DEVICE_FEATURES_SEL_OFFSET, i) = 1;
			usable_disks[i].device_features =
			    (uint64_t)(*R_LEVEL(
				VIRTIO_MMIO_DEVICE_FEATURES_OFFSET, i))
			    << 32;

			// LOW 32
			*R_LEVEL(VIRTIO_MMIO_DEVICE_FEATURES_SEL_OFFSET, i) = 0;
			usable_disks[i].device_features |=
			    *R_LEVEL(VIRTIO_MMIO_DEVICE_FEATURES_OFFSET, i);
		...
		usable_disks[i].blk_config.capacity =
		    (uint64_t)(*R_LEVEL(VIRTIO_MMIO_CONFIG_CAPACITY_HIGH_OFFSET,
					i))
			<< 32 |
		    *R_LEVEL(VIRTIO_MMIO_CONFIG_CAPACITY_LOW_OFFSET, i);
		usable_disks[i].blk_config.seg_max =
		    *R_LEVEL(VIRTIO_MMIO_CONFIG_SEG_MAX_OFFSET, i);
		usable_disks[i].blk_config.size_max =
		    *R_LEVEL(VIRTIO_MMIO_CONFIG_SIZE_MAX_OFFSET, i);
...
void free_desc(struct virtq_desc* chain_head)
{
	if (chain_head == NULL) {
		panic(PANIC_ERROR,
		      "virtio_blk_disk.c:free_desc(): chain_head is null!\n");
	}
...
void init_virtio_disk()
{
	...
	for (int i = 0; i < usable_device_count; i++) {
		...
		// 配置desc, avail, used
		// 选择队列
		b32_write(b_addr + VIRTIO_MMIO_QUEUE_SEL_OFFSET,
			  0); // QueueSel = 0

		// 设置大小
		b32_write(b_addr + VIRTIO_MMIO_QUEUE_NUM_OFFSET,
			  256); // QueueNum = 256

		// 写地址
		b32_write(b_addr + VIRTIO_MMIO_QUEUE_DESC_LOW_OFFSET,
			  GET_LOW_32(vq.desc_phy));
		b32_write(b_addr + VIRTIO_MMIO_QUEUE_DESC_HIGH_OFFSET,
			  GET_HIGH_32(vq.desc_phy));
		b32_write(b_addr + VIRTIO_MMIO_QUEUE_AVAIL_LOW_OFFSET,
			  GET_LOW_32(vq.avail_phy));
		b32_write(b_addr + VIRTIO_MMIO_QUEUE_AVAIL_HIGH_OFFSET,
			  GET_HIGH_32(vq.avail_phy));
		b32_write(b_addr + VIRTIO_MMIO_QUEUE_USED_LOW_OFFSET,
			  GET_LOW_32(vq.used_phy));
		b32_write(b_addr + VIRTIO_MMIO_QUEUE_USED_HIGH_OFFSET,
			  GET_HIGH_32(vq.used_phy));

		// 标记就绪
		b32_write(b_addr + VIRTIO_MMIO_QUEUE_READY_OFFSET,
			  1); // QueueReady = 1
		...
		printk(
		    "Disk %d: magic=%#x, version=%d, device_id=%d, vendor=%#x, "
		    "features=%#lx, driver_features=%#lx\n\t base=%#lx, "
		    "capacity=%lu (%lu KB, %lu MB, %lu GB), size_max=%u, "
		    "seg_max=%u\n",
		    i,
		    usable_disks[i].magic,
		    usable_disks[i].version,
