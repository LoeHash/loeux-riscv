...
static void init_slab_meta_pool()
{
        ...
        // 最后一个 slab_t
        char *last =
            mem_page + (slab_count - 1) * sizeof(slab_t);
        ...
        *(uint64_t *)last = 0;
        ...
        {
                inflate_slab_meta_pool(
                    SLAB_META_POOL_INIT_PAGE_SIZE - 1);
        }
}
...
static void inflate_slab_meta_pool(uint32_t page_size)
{
        ...
        while (page_size > 0)
        {
                ...
                {
                        char *slab =
                            mem_page + i * sizeof(slab_t);

                        char *next =
                            mem_page + (i + 1) * sizeof(slab_t);
        ...
        *(uint64_t *)last = 0;
        ...
        uint64_t added =
            (uint64_t)expand_page_size *
            (PG_4K_SIZE / sizeof(slab_t));
