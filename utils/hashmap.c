...
/// @param initial_size 期望的初始桶数量；小于最小值会被提升，非 2
/// 的幂会上取整。
...
hash_table_t*
hash_table_create(uint32_t initial_size,
		  ...
		  bool (*key_equal)(const void*, const void*, uint32_t))
{
	...
	ht->buckets =
	    (hash_node_t**)slab_alloc(sizeof(hash_node_t*) * initial_size);
	if (!ht->buckets) {
		slab_free(ht);
		panic_wrong(
		    "hash_table_create: slab_alloc(buckets, %u) failed\n",
...
static bool hash_table_resize(hash_table_t* ht, uint32_t new_size)
{
	...
	new_buckets =
	    (hash_node_t**)slab_alloc(sizeof(hash_node_t*) * new_size);
	if (!new_buckets)
		return false;
...
/// @return 成功插入或覆盖返回 true；参数非法、键冲突且不覆盖、节点分配失败返回
/// false。
		       ...
		       bool overwrite)
{
	...
	if ((ht->count + 1) * 100 / ht->size > HASH_TABLE_LOAD_FACTOR) {
		if (!hash_table_resize(ht, ht->size * 2)) {
			panic_wrong(
			    "hash_table_insert: resize %u -> %u failed\n",
			    ht->size,
			    ht->size * 2);
		}
		index = hash % ht->size;
...
/// @param key 键指针。
/// @param key_len 键的字节长度。
/// @return 找到返回保存的 value 指针（可能为 NULL）；未找到或参数非法返回
/// NULL。
...
bool hash_table_delete(hash_table_t* ht, const void* key, uint32_t key_len)
{
	...
	if (found && ht->size > HASH_TABLE_MIN_SIZE &&
		...
		if (!hash_table_resize(ht, ht->size / 2)) {
			panic_wrong(
			    "hash_table_delete: shrink %u -> %u failed\n",
			    ht->size,
			    ht->size / 2);
		}
	}
...
///        期间不会被其它线程插入同一个键。
///        等价于 hash_table_insert(ht, key, key_len, value,
///        /*overwrite=*/false)， 但语义上明确表示这是一次原子的
///        check-and-insert。
/// @param ht 目标哈希表；NULL、key 为 NULL、key_len 为 0 时返回 false。
/// @param key 键指针（不复制）。
/// @param key_len 键的字节长度。
...
bool hash_table_insert_if_absent(hash_table_t* ht,
				 const void* key,
				 uint32_t key_len,
				 void* value)
{
	...
	if ((ht->count + 1) * 100 / ht->size > HASH_TABLE_LOAD_FACTOR) {
		if (!hash_table_resize(ht, ht->size * 2)) {
			panic_wrong("hash_table_insert_if_absent: resize %u -> "
				    "%u failed\n",
				    ht->size,
				    ht->size * 2);
		}
	...
	if (!new_node) {
		panic_wrong(
		    "hash_table_insert_if_absent: slab_alloc(node) failed\n");
		release(&ht->lock);
		return false;
	}
