...
static void do_free_task_heap(page_table pg, uint64_t heap_start,
    uint64_t heap_history_max);
...
int set_cwd(struct task_struct *ts, const char *path) {
	...
	/*
	 * 文本层规范化（处理 . / .. / //）：
	 * "." ".." 不是合法的 FAT 目录项，VFS lookup
	 * 无法解析它们， 必须在进入 VFS
	 * 前消解成规范路径。 do_build_user_path 按
	 * BUFSZ 上限写入，用 slab 缓冲避免栈溢出。
	 */
	...
	do_build_user_path(normalized, (char *)path,
	    ts->cwd_path);

	/* 规范化失败（超长等）会得到空串；结果还必须放得进
	 * cwd_path */
	if (normalized[0] == '\0' ||
	    strlen(normalized) >=
	        sizeof(ts->cwd_path)) {
	...
	if (vfs_lookup(normalized, &inode) < 0) {
		slab_free(
		    normalized);
		return -1;
	}
...
void first_ret() {
	...
	uint64_t trampoline_userret =
	    TRAMPOLINE +
	    (_trampoline_ret - _trampoline_jump);
	((void (*)(uint64_t))trampoline_userret)(satp);
}

struct task_struct *get_task() {
	// 修复: get_task() 可能在中断开启的上下文（如
	// syscall 路径）中被调用。 若在 r_tp() 读取 tp
	// 之后、访问 cpus[tp].ts 之前发生定时器中断，
	// task 可能被迁移到其他 hart，sched() 中 w_tp()
	// 更新了 tp， 导致此处用旧 tp 索引到已被 drop
	// 的 cpus[旧hart].ts（=NULL），
	// 随后解引用引发内核态缺页。 用
	// intr_off/intr_on 保护 tp 读取与 cpus[tp].ts
	// 读取的原子性。
	int old = intr_get();
	intr_off();
...
void sched() {
	...
	// !!! DO NOT REMOVE !!!
	//
	// swtch 之后，当前 task 可能已经从其他 hart
	// 迁移到了本 hart。 调度器在 swtch 之前已经把
	// ts->utf->kernel_hartid 设置为 本 hart 的 hart
	// id，因此用它来修正 tp 寄存器。
	//
	// 否则 tp 仍然是旧 hart 的值，导致
	// get_cpu()/get_task() 访问 错误的 per-cpu
	// 数据，出现 cpu->ts == NULL、锁状态错乱、
	// 甚至内核态缺页等问题。
...
static void do_free_task_heap(page_table pg, uint64_t heap_start,
	...
	if (heap_start == 0)
		return;
	pg_unmap(pg, heap_start,
	    heap_history_max -
	        heap_start / PG_4K_SIZE,
	    1);
}
...
int kexit(int exit_code) {
	...
	do_free_task_heap(ts->pg, ts->heap_start,
	    ts->heap_history_max);
	...
	// 4. 孤儿收容：所有子进程改投 init。
	//    必须先释放自身锁：否则这里"持 ts->lk 再取
	//    child->lk" 与父进程 wait() 扫描"持
	//    child->lk 再取 ts->lk"会形成
	//    循环等待（典型场景：父子进程在不同 hart
	//    上同时退出）。 窗口期内 state 仍为
	//    RUNNING，父进程只会把它当作
	//    存活子进程计数，不会提前回收。
	release(&ts->lk);
	uint8_t orphan_zombie = 0;
	for (struct task_struct *c = tasks;
	    c < &tasks[NTASKS]; c++) {
		acquire(&c->lk);
		if (c->parent ==
		    ts) {
	...
	// 5. 最后才置 ZOMBIE：
	//    父进程只能通过 acquire(ts->lk) 观察到
	//    ZOMBIE，而本锁会 一直持有到 sched()
	//    切出、由调度器释放——这保证父进程
	//    真正拿到锁开始回收时，本任务已彻底停止运行
	//    若过早置 ZOMBIE，父进程可能在 trapframe
	//    页仍被 本任务使用时就
	//    kfree(utf)，甚至把槽位置回 INITLIZED 交给
	//    alloc_task
	//    复用——而本任务还在自己的内核栈上跑着。
	ts->state = ZOMBIE;
	ts->dead = 1;
...
int kfork() {
	...
	if ((vm_pagetbl_copy_asign(
	        father_ts->pg, new_ts->pg,
	        USER_BASE_PROG_ADDR,
	        father_ts->size -
	            USER_BASE_PROG_ADDR)) == -1) {
		// 失败路径必须释放锁，否则
		// free_task
		// 之后该槽位被复用，
		// 后续 acquire
	...
	if ((vm_pagetbl_copy_asign(
	        father_ts->pg, new_ts->pg,
	        father_ts->heap_start,
	        (father_ts->heap_history_max -
	            father_ts->heap_start))) == -1) {
		release(
		    &new_ts
		        ->lk);
		free_task(
	...
	*(new_ts->utf) = *(father_ts->utf);
	...
	// 子进程可能已被其他 hart 调度、甚至 exit
	// 并被回收。 此处绝不能再解引用 new_ts
	return new_ts->pid;
}
...
int kexec(char *path, char **argv) {
	...
	// 因此提到最前面声明，保证任何 goto out
	// 都可见。
	uint64_t old_size = t->size, new_size = 0,
	         old_heap_start = t->heap_start,
	         old_heap_history_max =
	             t->heap_history_max;

	// ret 默认
	// -1（失败）；仅在真正提交（替换页表）时改写为
	// argc。 fd/buf/ef 初始化为“未持有”，out
	// 里据此安全释放。
	int ret = -1;
	int fd = -1;
	char *buf = NULL;
	...
	if (copyout(
	        new_page, new_sp, (char *)user_argv_ptr,
	...
	strcpy_with_terminate(t->name, last,
	    sizeof(t->name));
	...
	t->size = new_size;
	t->utf->sp = new_sp;
	t->utf->a0 = argc; // crt0 的 _start 直接 call
		   // main，main 从 a0 读 argc
	...
	// 页表所有权已转移给 t->pg，out 不能再释放
	// new_page
	new_page = 0;

	free_task_pgtable(old_page, old_size);

	do_free_task_heap(old_page, old_heap_start,
	    old_heap_history_max);

	ret = argc;
out:
	// 统一清理：失败不 panic，且不泄漏 fd / buf /
	// 半成品页表。 旧页表 old_page
	// 不在此释放（成功路径已在提交后单独释放）。
	if (buf) {
		free_page(buf);
...
static int load_segment(struct file *f, page_table pg, struct elf64_phdr *ph) {
	...
	uint64_t start =
	    PGROUNDDOWN(ph->p_vaddr); // 段起始页
	uint64_t end = PGROUNDUP(
	    ph->p_vaddr + ph->p_memsz); // 段结束页

	// BSS 边界：p_memsz > p_filesz 时，超出 filesz
	// 的部分是 BSS（零填充、可写）。 链接器可能把
	// .text 和 .bss 放进同一个 LOAD 段，整体标为
	// R+E（无 W）， 但 BSS
	// 必须可写。file_end_round_down
	// 是最后一个含文件数据页的起始，
	// 从该页起（含）需要加 PTE_W——因为该页内
	// file_end 之后就是 BSS。
	uint64_t file_end = ph->p_vaddr + ph->p_filesz;
	uint64_t bss_page_start =
	    PGROUNDDOWN(file_end); // 第一个含 BSS 的页
...
static int check_elf_header(struct elf64_ehdr *ehdr) {
	...
	// 检查端序
	if (ehdr->e_ident[EI_DATA_OFFSET] !=
	    EI_DATA_VAL) {
		return -3;
	}

	// 检查class
	if (ehdr->e_ident[EI_CLASS_OFFSET] !=
	    EI_CLASS_VAL) {
		return -2;
	}

	// 检查版本
	if (ehdr->e_ident[EI_VERSION_OFFSET] !=
	    EI_VERSION_VAL) {
		return -4;
	}
...
static void _map_user_stack(page_table pg) {
	for (uint64_t i = USER_STACK_BASE;
	    i < USER_STACK_TOP; i += PG_4K_SIZE) {
		mappages(
		    pg, i,
		    PG_4K_SIZE,
		    (uint64_t)
...
/// @brief 持锁睡下。
///   调用方持有 lock 保护要等的条件 和 t->lk，
///   在 lock 内检查条件，条件不成立时调用本函数。
///   本函数负责：标记 sleep_chan、设 SLEEP、释放 lock、
///   切走。被唤醒后重新获取 lock，返回。
///
/// 返回后：调用方持有 lock 和 t->lk，继续在 lock 内检查条件。
/// @param chan 睡眠通道，wakeup 用同样的 chan 唤醒。
/// @param lock 保护调用方条件的锁。调用时已持有，返回时已重获。
void sleep_locked(void *chan, spinlock_t *lock) {
	...
	// 释放外部锁，让改条件的人能进来、能 wakeup。
	release(lock);

	// 切走。sched 返回时 t->lk 仍然持有。
	// 切走期间 CPU 会去跑别的进程，直到有人 wakeup
	// 本进程， 调度器再次选中它，从这里继续。
	sched();
...
/// @brief 唤醒所有 sleep 在 chan 上的进程。
///        只在进程上下文调用，调用时当前进程一定持有自己的 t->lk，
///        因此遍历到自己时跳过，避免自死锁。
/// @param chan 睡眠通道。
void _wakeup(void *chan)
{
    struct task_struct *self = get_task();
    ...
    for (t = tasks; t < &tasks[NTASKS]; t++) {
        ...
        if (t->state == SLEEP && t->sleep_chan == chan) {
            t->state      = RUNNABLE;
            t->sleep_chan = NULL;
        }
