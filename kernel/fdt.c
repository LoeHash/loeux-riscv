...
// this is callback!
int walk_fdt(const char* name, int depth, void* node_ptr, void* data)
{
	// 打印缩进
...
int read_node_prop(uint8_t* start, char* prop_name, void* dest, int _size)
{
	...
	if (len > CHAR_BUFF_SIZE - 1) {
		printk("read_node_prop: excepted the length of node prop name "
		       "less than %u!\n",
		       CHAR_BUFF_SIZE);
		return -1;
	}
...
void read_node_name(uint8_t* start, char* buf)
{
	int len = strlen(start + 4);
	if (len > CHAR_BUFF_SIZE - 1) {
		printk("read_node_name: excepted the length of node less than "
		       "%u!\n",
		       CHAR_BUFF_SIZE);
		return;
	}
...
void fdt_header_init()
{
	...
	while (bte32(root_ptr) == FDT_PROP) {
		...
		root_ptr =
		    (uint8_t*)ALIGN_UP((uintptr_t)(root_ptr + prop_len), 4);
		printk("=================================\n");
	}
