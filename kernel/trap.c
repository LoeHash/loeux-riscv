...
void kernel_trap_hanlder(uint64_t scause, uint64_t sepc, uint64_t stval)
{
	...
	if (r_sstatus_spp() != 1) {
		printk("KERNEL TRAP RAW: tp=%lx hart=%d scause=%lx sepc=%lx "
		       "satp=%lx stvec=%lx sstatus=%lx\n",
		       r_tp(),
		       get_cpu_id(),
		       scause,
		       sepc,
		...
		panic(PANIC_ERROR,
		      "kernel_trap_hanlder: NOT FROM THE KERNEL!\n");
	}
	...
	if (scause == CLINT_INTERRUPT_SCAUSE) {
		...
		// 在内核态里的时钟中断
		do_timer_tick();
		sbi_set_timer(rdtime() +
			      (BASE_FREQUENCY / TIMER_TICKS_PER_SEC));
		if (ts == 0) {
			w_sepc(sepc);
			w_sstatus(sstatus);
			return;
		...
		if (ts->trap_ctx_valid) {
			w_sepc(
			    ts->trap_sepc); // 恢复 task 原来的 kernel trap PC
			w_sstatus(ts->trap_sstatus); // 恢复 task 原来的 kernel
						     // trap sstatus
		}
...
uint64_t user_trap_hanlder(uint64_t scause, uint64_t sepc, uint64_t stval)
{
	...
	if (scause == 8) // syscall
	{
		...
		sbi_set_timer(rdtime() +
			      (BASE_FREQUENCY / TIMER_TICKS_PER_SEC));
		yield();
	} else if (scause == PAGE_FAULT_LOAD_SCAUSE ||
