...
int vfs_register_chardev(const char* name,
			 ...
			 void* priv)
{
	...
	if (!hash_table_insert_if_absent(
		char_dev_map, name, cdev_key_len(name), cdev)) {
		slab_free(cdev);
...
struct char_device* vfs_find_chardev(const char* name)
{
	...
	return (struct char_device*)hash_table_lookup(
	    char_dev_map, name, cdev_key_len(name));
}