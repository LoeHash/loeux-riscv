...
struct ext2_superblock
{
        ...
        uint16_t s_inode_size;      /* 偏移 88：每个磁盘 inode 的大小（128/256...） */
...
static int ext2_load_super(struct ext2_fs_priv *fs,
                           struct ext2_superblock *sb)
{
        ...
        memcpy(sb, buf + (fs->block_size == 1024 ? 0 : 1024 % fs->block_size),
               sizeof(*sb));
...
static int ext2_load_bg(struct ext2_fs_priv *fs,
                        uint32_t group,
                        struct ext2_bg_desc *out)
{
...
static int ext2_bitmap_test(struct ext2_fs_priv *fs,
...
static int ext2_bitmap_set(struct ext2_fs_priv *fs,
                           uint32_t bitmap_block,
                           uint32_t bit)
{
        uint8_t *buf;
...
static int ext2_bitmap_clear(struct ext2_fs_priv *fs,
                            uint32_t bitmap_block,
                            uint32_t bit)
{
        uint8_t *buf;
...
static int ext2_inode_read(struct ext2_fs_priv *fs,
                           uint32_t ino,
                           struct ext2_inode *out)
{
        ...
        inode_block = bg.bg_inode_table + (idx * fs->inode_size) / fs->block_size;
...
static int ext2_alloc_block(struct ext2_fs_priv *fs, uint32_t *out)
{
        ...
        {
                struct ext2_bg_desc bg;
                uint32_t blk_base = fs->first_data_block +
...
/* 把 priv 回写到磁盘 inode */
static int ext2_priv_save(struct ext2_fs_priv *fs,
                          struct ext2_inode_priv *priv)
{
                         ...
                         struct ext2_dir_scan *out)
{
        ...
        {
                uint32_t phys = 0;
                uint32_t start_off = (lblk == start_blk)
                                     ? out->offset % fs->block_size
                                     : 0;
                ...
                while (off + sizeof(struct ext2_dirent) <= fs->block_size)
                {
                        ...
                        if (off >= start_off)
                        {
                                if (e->inode != 0)
                                {
                                        ...
                                        out->next_off = lblk * fs->block_size + off + rec;
                                        out->found = 1;
                                        slab_free(buf);
                                        return 0;
                                }
                        ...
                        uint8_t file_type)
{
        uint8_t *buf;
        uint32_t max_blk;
        uint16_t need = (uint16_t)((sizeof(struct ext2_dirent) + name_len + 3) & ~3u);
        uint32_t lblk;
        uint32_t added = 0;
        ...
        for (lblk = 0; lblk < max_blk && !added; lblk++)
        {
                ...
                while (off + sizeof(struct ext2_dirent) <= fs->block_size)
                {
                        ...
                        if (e->inode == 0 && e->rec_len >= need)
                        {
                                ...
                                if (rest >= (uint16_t)sizeof(struct ext2_dirent))
                                {
                                        /* 分割：尾部继续作为 inode=0 的空闲项 */
                                        e->rec_len = need;
                                        struct ext2_dirent *restp =
                                                (struct ext2_dirent *)(buf + off + need);
                                        restp->inode = 0;
                                        restp->rec_len = rest;
                                        restp->name_len = 0;
                                        restp->file_type = 0;
                ...
                if (!added)
                {
                        ...
                        if (last_rec > 0)
                        {
                                ...
                                uint16_t real_len = (uint16_t)((sizeof(struct ext2_dirent) +
                                                                last->name_len + 3) & ~3u);
                                ...
                                if (tail >= need)
                                {
                                        ...
                                        struct ext2_dirent *ne =
                                                (struct ext2_dirent *)(buf + last_off + real_len);
                                        ne->inode = ino;
                                        ne->rec_len = tail;
        ...
        for (uint32_t lblk = 0; lblk < max_blk; lblk++)
        {
                ...
                while (off + sizeof(struct ext2_dirent) <= fs->block_size)
                {
                            ...
                            memcmp(e->name, name, name_len) == 0)
                        {
                                ...
                                {
                                        /* 找上一个 entry：从头扫到上一个 off 之前的累计 */
                                        uint32_t p = 0;
                                        while (p < off)
                                        {
                                                struct ext2_dirent *pe =
                                                        (struct ext2_dirent *)(buf + p);
...
static int ext2_dir_empty(struct ext2_fs_priv *fs,
                          struct ext2_inode_priv *dir)
{
        uint8_t *buf;
        uint32_t max_blk;
                             ...
                             uint32_t parent_ino)
{
        ...
        struct ext2_dirent *ddot =
                (struct ext2_dirent *)(buf + 12);
...
static int ext2_lookup(struct inode *dir,
                       const char *name,
                       struct inode **out)
{
        struct ext2_fs_priv *fs;
        ...
        if (dir == NULL || name == NULL || out == NULL ||
            dir->sb == NULL || dir->sb->private == NULL ||
            dir->private == NULL)
                return -1;
                       ...
                       struct inode **inode)
{
        ...
        /* 把目录项加入父目录 */
        if (ext2_dir_add(fs, dp, name, name_len, new_ino, EXT2_FT_REG_FILE) < 0)
        {
                ext2_free_inode(fs, new_ino);
                return -1;
        }
...
static int ext2_readdir(struct inode *dir,
                        uint64_t *offset,
                        struct vfs_dirent *dirent)
{
        ...
        dirent->ino = scan.ino;
        dirent->type =
                (scan.type == EXT2_FT_DIR) ? S_IFDIR :
                (scan.type == EXT2_FT_REG_FILE) ? S_IFREG :
                (scan.type == EXT2_FT_SYMLINK) ? S_IFLNK : S_IFREG;
...
static int ext2_getattr(struct inode *inode, struct vfs_kstat *stat)
{
        ...
        if (inode == NULL || stat == NULL ||
            inode->sb == NULL || inode->private == NULL)
                return -1;
                          ...
                          uint32_t *out_len)
{
        ...
        while (remaining > 0)
        {
                ...
                uint32_t n = (uint32_t)(remaining < (uint64_t)(fs->block_size - boff)
                                        ? remaining
                                        : (fs->block_size - boff));
                uint32_t phys = 0;
                           ...
                           uint32_t *out_len)
{
        ...
        while (f < end)
        {
                ...
                uint32_t n = (uint32_t)((end - f) < (uint64_t)(fs->block_size - boff)
                                        ? (end - f)
                                        : (fs->block_size - boff));
                uint32_t phys = 0;
