...
static int fat32_read_sector(struct fat32_fs_priv *fs,
                             uint64_t sector,
                             void *buf)
{
...
static int fat32_write_sector(struct fat32_fs_priv *fs,
                              uint64_t sector,
                              const void *buf)
{
...
/* 读写 FAT 表项（低28位有效） */
static int fat32_fat_get(struct fat32_fs_priv *fs,
                         uint32_t cluster,
                         uint32_t *out)
{
        uint32_t sec;
...
static int fat32_fat_set(struct fat32_fs_priv *fs,
                         uint32_t cluster,
                         uint32_t value)
{
        uint32_t sec;
...
static int fat32_alloc_cluster(struct fat32_fs_priv *fs, uint32_t *out)
{
        ...
        for (c = 2; c < fs->cluster_count + 2; c++)
        {
                ...
                {
                        if (fat32_write_sector(fs,
                                               fat32_cluster_to_sector(fs, c) + i,
                                               zero) < 0)
                        {
...
static void fat32_unix_to_fat(uint32_t ux_sec,
                              struct fat32_fat_time *out)
{
...
static void fat32_dos_to_name(const uint8_t raw[11],
                              uint8_t ntres,
                              char *out)
{
        int base_len = 0;
...
static int fat32_dir_walk(struct inode *dir, fat32_dir_cb cb, void *arg)
{
               ...
               guard++ < fs->cluster_count)
        {
                for (uint32_t s = 0; s < fs->sectors_per_cluster; s++)
                {
                        ...
                        for (uint32_t i = 0; i < fs->bytes_per_sector / FAT32_DIRENT_SIZE; i++)
                        {
                                struct fat32_dirent32 *e =
                                        (struct fat32_dirent32 *)(buf + i * FAT32_DIRENT_SIZE);
                                uint32_t off = s * fs->bytes_per_sector + i * FAT32_DIRENT_SIZE;
                                int r;
                                        ...
                                        uint32_t dirent_off)
{
        ...
        if (e->dir_attr & FAT32_ATTR_DIRECTORY)
        {
                ...
                inode->size = fat32_chain_len(fs, first) * fat32_cluster_bytes(fs);
        }
        else
        {
                uint32_t perm = 0644;
...
static struct inode *fat32_root_inode(struct super_block *sb)
{
        ...
        inode->mode = S_IFDIR | 0755;
        inode->size = fat32_chain_len(fs, fs->root_cluster) *
                      fat32_cluster_bytes(fs);
        inode->iops = &fat32_inode_ops;
        inode->fops = &fat32_file_ops;
        inode->private = priv;
        inode->refcount = 1;
...
static int fat32_lookup(struct inode *dir,
                        const char *name,
                        struct inode **out)
{
...
static int fat32_readdir(struct inode *dir,
                         uint64_t *offset,
...
static int fat32_getattr(struct inode *inode, struct vfs_kstat *stat)
{
        ...
        stat->nlink = 1;
        stat->blocks = (inode->size + fs->bytes_per_sector - 1) /
                       fs->bytes_per_sector;
                           ...
                           struct super_block **out)
{
        ...
        if (bpb->bpb_bytes_per_sector < 512 ||
            ...
            (bpb->bpb_bytes_per_sector &
             (bpb->bpb_bytes_per_sector - 1)) != 0)
                goto free_sec;
